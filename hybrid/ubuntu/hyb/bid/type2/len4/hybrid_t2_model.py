...
class Encoder(nn.Module):
	def __init__(self, emb_size, enc_hidden_size):
		super(Encoder,self).__init__()
		self.emb_size = emb_size
		self.enc_hidden_size = enc_hidden_size

		self.gru = nn.GRU(self.emb_size,self.enc_hidden_size,bidirectional=True,batch_first=True)

	def forward(self, input, hidden):
		'''input&output: (batch_size,max_senten_len,emb_size)
		hidden: (1,batch_size,enc_hidden_size*2)'''
...
class Seq2Seq(nn.Module):
	def __init__(self, dict_size, emb_size, enc_hidden_size, batch_size, dropout, max_senten_len, teach_forcing):
		...
		self.encoder = Encoder(self.emb_size, self.enc_hidden_size)
		self.decoder = Decoder(self.dict_size, self.enc_hidden_size*2, self.emb_size)
		self.topic_attention = TopicAttention(self.enc_hidden_size*2)
	...
	def forward(self, reply_tensor_batch, contexts_tensor_batch, pad_matrix_batch, ini_idx):
		...
		encoder_hidden = Variable(torch.zeros(2,self.batch_size,self.enc_hidden_size)).cuda()
		list_context_hidden = []
		list_decoder_output = []
		...
		for context_tensor in contexts_tensor_batch:
			...
			# context_hidden: 1*batch_size*enc_hidden_size(2)
		...
		# context_hiddens: batch_size*context_size*enc_hidden_size*2
		context_hiddens = torch.cat(tuple(list_context_hidden),0).transpose(0,1)
		# topic_vector: batch_size*1*enc_hidden_size*2
	...
	def predict(self, contexts_tensor_batch, reply_index2word, pad_matrix_batch, ini_idx, sep_char=''):
		encoder_hidden = Variable(torch.zeros(2,self.batch_size,self.enc_hidden_size),volatile=True).cuda()
